...
#else
void Widgets::FileDialog::fillRoots() { m_roots.push_back({MAKEU8(u8"/"), "(root)"}); }
#endif
...
bool FileDialog::draw() {
    ...
    if (ImGui::BeginPopupModal(m_title(), NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (m_flags & NewFile) {
            ...
            selectedStr = (m_currentPath / std::filesystem::path(selected ? selected->filename : std::string(u8"..."))).u8string();
